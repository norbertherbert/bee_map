...
mosquitto_pub -h test.mosquitto.org -p 1883 -t bee_map/20635FF200007E8A -f example.json

# or test with http POST (curl)
# this test requires lainching the backend server (in the backend folder)
curl -X POST http://localhost:4000/ingest -H "Content-Type: application/json" --data-binary @example.json
